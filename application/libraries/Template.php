...
class Template {
    ...
    /* Definicion de variables para SEO */
    var $title = "";
    var $description = "";
    var $keywords = "";
    ...
    function add_js($script, $type = 'import', $defer = FALSE) {
        ...
        switch ($type) {
            case 'import':
                $filepath = base_url() . 'public/js/' . $script;
                $js = '<script type="text/javascript" src="' . $filepath . '?' . $num . '"';
                if ($defer) {
                    $js .= ' defer="defer"';
                }
                $js .= "></script>";
    ...
    function add_css($style, $type = 'link', $media = FALSE) {
        ...
        $this->CI->load->helper('url');
        $filepath = base_url() . 'public/css/' . $style;
